...
class MySQL {
  ...
  public function where($where_key = null, $where_value = null) {
    if (is_array($where_key)) {
      ...
      $where_sql_value[$where_key] = MySQL::SQLValue($where_value);
    }
  ...
  /**
   * [STATIC] Builds a SQL COUNT statement
   *
   * @param string $tableName The name of the table
   ...
   * @return string Returns the SQL COUNT statement
   */
  static public function BuildSQLCount($tableName, $whereArray = null) {
  ...
  public function count($count) {
    $tableName  = $this->table;
    $whereArray = $this->where;
    $sql        = self::BuildSQLCount($tableName, $whereArray);
    $result     = $this->Query($sql);
    if (!$result) {
      return false;
    } else {
