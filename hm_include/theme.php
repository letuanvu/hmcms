...
function css($file = NULL, $attr = array()) {
    if (is_array($file)) {
        ...
        foreach ($file as $sub_file) {
            $return .= build_css($sub_file, $attr);
        }
        return $return;
    } else {
        return build_css($file, $attr);
    }
}
function build_css($file = NULL, $attr = array()) {
    ...
    if ($is_url == FALSE) {
        if (is_numeric($file)) {
            ...
            if (file_exists($file_local)) {
                ...
                if ($file_time != $old_file_time || isset($_GET['clean'])) {
                    /** xóa css cũ */
                ...
                return '<!-- CSS FILE LOST <link rel="stylesheet" type="text/css" href="' . BASE_URL . HM_THEME_DIR . '/' . $theme . '/' . $file_local . '"' . $attr_str . '> -->' . "\n";
                ;
            }
        }
    } else {
...
function js($file = NULL, $attr = array()) {
    ...
    if ($is_url == FALSE) {
        if (is_numeric($file)) {
            $file_url = get_file_url($file);
            if ($file_url) {
                return '<script src="' . $file_url . '"' . $attr_str . '></script>' . "\n";
            } else {
                return FALSE;
            }
        } else {
            ...
            if (file_exists($file_local)) {
                ...
                return '<script src="' . BASE_URL . HM_THEME_DIR . '/' . $theme . '/' . $minjs_file_name . '"' . $attr_str . '></script>' . "\n";
            } else {
                return FALSE;
            }
            $theme            = activated_theme();
        ...
        return '<script src="' . $file . '"' . $attr_str . '></script>' . "\n";
    }
}
...
function cssbyjs($files = array()) {
    ...
    $str_css = implode("',\n\r  '", $files_fix);
    echo "<script> \n\r" . "[ \n\r" . "  '" . $str_css . "' \n\r" . "].forEach(function(href) { \n\r" . "  var script = document.createElement('link'); \n\r" . "  script.href = href; \n\r" . "  script.rel = 'stylesheet'; \n\r" . "  script.type = 'text/css'; \n\r" . "  script.async = false; \n\r" . "  document.head.appendChild(script); \n\r" . "}); \n\r" . "</script> \n\r";
}
function cssdeferred($files = array(), $all_in_one = FALSE, $attr = array()) {
    ...
    if ($all_in_one == TRUE) {
        ...
        if (!file_exists($all_in_one_local) || isset($_GET['clean'])) {
            foreach ($files as $file) {
                $file_local = './' . HM_THEME_DIR . '/' . $theme . '/' . $file;
                if (isset($file_local)) {
                    $content_css .= '/** START ' . $file . ' **/' . "\n\r";
                    $content_css .= file_get_contents($file_local);
                    $content_css .= '/** END ' . $file . ' **/' . "\n\r";
                }
            }
    ...
    echo '</noscript>' . "\n\r";
    echo "<script> \n\r" . "	var loadDeferredStyles = function() { \n\r" . "	  var addStylesNode = document.getElementById('hm-deferred-styles'); \n\r" . "	  var replacement = document.createElement('div'); \n\r" . "	  replacement.innerHTML = addStylesNode.textContent; \n\r" . "	  document.body.appendChild(replacement); \n\r" . "	  addStylesNode.parentElement.removeChild(addStylesNode); \n\r" . "	}; \n\r" . "	var raf = requestAnimationFrame || mozRequestAnimationFrame || \n\r" . "		webkitRequestAnimationFrame || msRequestAnimationFrame; \n\r" . "	if (raf){ raf(function() { window.setTimeout(loadDeferredStyles, 0); }); \n\r" . "	}else{ window.addEventListener('load', loadDeferredStyles); } \n\r" . "</script> \n\r";
}
...
function hm_title() {
    ...
    if ($request == '') {
        /** Trang chủ */
        $hm_title = get_option(array(
            'section' => 'system_setting',
            'key' => 'website_name',
            'default_value' => 'Trang chủ'
        ));
    } else {
        ...
        if ($registed_request != FALSE AND is_array($registed_request)) {
            ...
            if ($request == '') {
                $home_title = get_option(array(
                    'section' => 'system_setting',
                    'key' => 'website_name',
                    'default_value' => 'Một trang web sử dụng HoaMaiCMS'
                ));
                if ($home_title != '') {
                    $hm_title = $home_title;
                } else {
...
function live_image($default_img, $key = '') {
    ...
    if (is_admin_login()) {
        ...
        if (trim($href) != '') {
            echo '<a href="' . $href . '" title="' . $alt_tag . '"><img src="' . $value . '" alt="' . $alt_tag . '" /></a>';
        } else {
            echo '<img src="' . $value . '" alt="' . $alt_tag . '" />';
        }
    }
}
function live_ahref($value, $key = '') {
    ...
    $value            = str_replace('{link}', $href, $value);
    $value            = str_replace('{url}', $href, $value);
...
function live_option($input, $mes = '') {
    $theme       = activated_theme();
    $bt          = debug_backtrace();
    $caller      = array_shift($bt);
    $file        = $caller['file'];
    $line        = $caller['line'];
    $basename    = basename($file, '.php');
    $option_name = $theme . '_' . $basename . '_' . md5(json_encode($input));
    if (!is_array($input[0])) {
        $input = array(
            $input
        );
    ...
    if (is_admin_login()) {
        $_SESSION['live_option_input'][$option_name] = $input;
        echo '<live_option data-option_name="' . $option_name . '">' . "\n";
        echo $mes . "\n";
        echo '</live_option>' . "\n";
    }
}
function live_menu($key = '', $args_menu = array()) {
    $theme    = activated_theme();
    $bt       = debug_backtrace();
    $caller   = array_shift($bt);
    $file     = $caller['file'];
    ...
    if (is_admin_login()) {
        if (is_numeric($value)) {
            $args_menu['parent'] = $value;
            echo get_menu($args_menu);
        }
        echo '<live_menu data-option_name="' . $option_name . '">' . "\n";
        echo '<form action="' . BASE_URL . 'admin/?run=live_edit.php&live_edit=on&type=menu" method="post" class="hm_live_option_form">';
        $args = array(
            'name' => $option_name,
            'input_type' => 'menu',
            'required' => FALSE,
