...
function build_input_form($field_array) {
    ...
    if ($field_array['name']) {
        if (isset($field_array['required']) AND ($field_array['required'] == TRUE))
...
function input_radio($field_array = array()) {
    ...
    echo '<ul class="option_list tree">' . "\n";
...
function build_input_slug($args) {
    ...
    if (is_numeric($object_id) AND $object_id != 0) {
        if ($object_type == 'taxonomy') {
            $slug    = get_tax_val(array(
                'name' => 'slug_of_' . $val_name,
                'id' => $object_id
            ));
            $checked = get_tax_val(array(
                'name' => 'accented_of_' . $val_name,
                'id' => $object_id
            ));
        }
        if ($object_type == 'content') {
            $slug    = get_con_val(array(
                'name' => 'slug_of_' . $val_name,
                'id' => $object_id
            ));
            $checked = get_con_val(array(
                'name' => 'accented_of_' . $val_name,
                'id' => $object_id
            ));
        }
...
function input_time($field_array = array()) {
    $name_prefix = '';
    if (isset($field_array['name_prefix'])) {
        $name_prefix = $field_array['name_prefix'];
    }
    if (!isset($field_array['default_value']))
    ...
    $selected = '';
    echo '		<input type="text" name="' . $name_prefix . 'day" id="day" value="' . $day . '" />';
    echo '		<select name="' . $name_prefix . 'month" id="month" class="col-md-6">';
    if ($month == '01') {
        $selected = 'selected';
    } else {
        $selected = '';
    ...
    echo '		</select>';
    echo '		<input type="text" name="' . $name_prefix . 'year" id="year" value="' . $year . '" />';
    echo '		<span>@</span>';
    echo '		<input type="text" name="' . $name_prefix . 'hour" id="hour" value="' . $hour . '" />';
    echo '		<span> : </span>';
    echo '		<input type="text" name="' . $name_prefix . 'minute" id="minute" value="' . $minute . '" />';
}
