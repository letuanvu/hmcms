...
function pagination($args = array()) {
    ...
    if ($total_result > 0) {
        ...
        $pagination_id    = '';
        if ($args['class']) {
            $pagination_class = ' class="' . $args['class'] . '"';
        }
        if ($args['id']) {
            $pagination_id = ' id="' . $args['class'] . '"';
        }
...
function content_breadcrumb($id, $args = array()) {
    ...
    if ($hmdb->HasRecords()) {
        ...
        $main_taxonomy  = get_con_val("name=main_taxonomy&id=$id");
        $taxonomy       = json_decode($taxonomy, TRUE);
        if (is_numeric($main_taxonomy)) {
            $tax_bre = taxonomy_parent_breadcrumb($main_taxonomy, 1);
...
function content_thumbnail($args = array()) {
    ...
    if (is_numeric($content_thumbnail)) {
        ...
        if (is_numeric($w) AND is_numeric($h)) {
            $display_src = create_image(array(
                'file' => $content_thumbnail,
                'w' => $w,
                'h' => $h,
                'crop' => $crop
            ));
