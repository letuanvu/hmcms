...
function get_use_val($args = array()) {
    if (!is_array($args)) {
        parse_str($args, $args);
    }
    $args = hook_filter('before_get_use_val', $args);
    global $hmuser;
    hook_action('get_use_val');
    $return = $hmuser->user_field_data(array(
        'id' => $args['id'],
        'field' => $args['field']
    ));
    $return = do_all_shortcode_from_string($return);
    $return = hook_filter('after_get_use_val', $return, $args);
    return $return;
}
?>
