...
function available_plugin() {
    ...
    foreach ($available_plugin as $plugin) {
        if ((is_dir(BASEPATH . HM_PLUGIN_DIR . '/' . $plugin)) AND (is_file(BASEPATH . HM_PLUGIN_DIR . '/' . $plugin . '/' . $plugin . '.php'))) {
...
function is_plugin_active($key) {
    ...
    if ((is_dir(BASEPATH . HM_PLUGIN_DIR . '/' . $key)) AND (is_file(BASEPATH . HM_PLUGIN_DIR . '/' . $key . '/' . $key . '.php'))) {
...
function add_plugin($href = FALSE) {
    if (is_url_exist($href)) {
        ...
        return hm_json_encode(array(
            'status' => 'error',
            'mes' => hm_lang('can_not_connect_to_the_download_server')
        ));
    }
}
?>
