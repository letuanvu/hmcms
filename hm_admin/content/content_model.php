...
function content_ajax_add($args = array()) {
    ...
    if (($key != NULL) AND isset($con[$key])) {
            ...
            if (is_numeric($insert_id)) {
                ...
                if ($number_order == 1) {
                    ...
                    foreach ($content_update_ids as $content_update_id) {
                        $old_number_order = get_con_val(array(
                            'name' => 'number_order',
                        ...
                        update_con_val(array(
                            'name' => 'number_order',
                            'id' => $content_update_id,
                            'value' => $new_number_order
                        ));
                        $values                  = array();
                        $values["content_order"] = MySQL::SQLValue($new_number_order);
                        $whereArray              = array(
                            'id' => MySQL::SQLValue($content_update_id)
                        );
        ...
        if (is_numeric($insert_id)) {
            ...
            if (!is_numeric($year))
                $year = 0;
            $hour = $input_post['hour'];
            if (!is_numeric($hour))
                $hour = 0;
            $minute = $input_post['minute'];
...
function content_show_data($key, $args) {
    ...
    if (sizeof($contents) > 0) {
        ...
        foreach ($contents as $cid) {
            ...
            $slug              = $row->slug;
            $status            = $row->status;
            $name              = get_con_val(array(
                'id' => $cid,
                'name' => 'name'
            ));
            $number_order      = get_con_val(array(
                'id' => $cid,
                'name' => 'number_order'
            ));
            $content_thumbnail = get_con_val(array(
                'id' => $cid,
...
function content_box($args = array()) {
    ...
    foreach ($content_box as $box) {
        if (!isset($box['content_key'])) {
            $box['content_key'] = NULL;
        }
        if ($box['content_key'] == $content_key OR (!isset($box['content_key']))) {
            ...
            if ($box['position'] == $position) {
                $class = '';
                if ($position == 'left') {
                    $class      = 'admin_mainbar_box';
                    $data_order = mainbar_box_order($func);
                }
                if ($position == 'right') {
                    $class      = 'admin_sidebar_box';
                    $data_order = sidebar_box_order($func);
                }
