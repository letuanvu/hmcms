...
<div class="row" >
  ...
  <form autocomplete="off" action="?run=content_ajax.php&id=<?php echo hm_get('id'); ?>&action=edit" method="post" class="ajaxForm ajaxFormcontentEdit">
    ...
    <div class="col-md-3 admin_sidebar">
      ...
      <div class="row admin_sidebar_box" data-func="hm_content_info" data-order="<?php echo sidebar_box_order('hm_content_info'); ?>">
        ...
        <div class="admin_mainbar_boxcontent" data-func="hm_content_info">
          ...
          <div class="form-group">
            <label for="status">
              <span class="glyphicon glyphicon-calendar"></span> <?php echo hm_lang('public_time'); ?>
            </label>
          ...
          <div class="form-group">
            <label for="status">
              <span class="glyphicon glyphicon-calendar"></span> <?php echo hm_lang('update_time'); ?>
            </label>
            <div class="change_content_time">
              <?php
              $field_array = array();
              $field_array['name_prefix'] = 'update_';
              input_time($field_array);
              unset($field_array);
              if(isset($fields_val['update_time'])){
                $update_time = $fields_val['update_time'];
              }else{
                $update_time = $fields_val['public_time'];
              }
              ?>
              <p class="input_description"><?php echo hm_lang('last_update'); ?>: <?php echo date(DATE_FORMAT, $update_time); ?></p>
            </div>
          </div>
          <div class="form-group">
            <label for="revision">
      ...
      if($args_tax){
        ?>
        <div class="row admin_sidebar_box" data-func="hm_main_taxonomy" data-order="<?php echo sidebar_box_order('hm_content_taxonomy'); ?>">
          <p class="admin_sidebar_box_title" data-func="hm_main_taxonomy"><?php echo hm_lang('main_taxonomy'); ?></p>
          <div class="admin_mainbar_boxcontent" data-func="hm_main_taxonomy">
            <?php
            $field_array                  = array();
            $field_array['input_type']    = 'taxonomy_select';
            $field_array['name']          = 'main_taxonomy';
            $field_array['data_key']      = array($args['taxonomy_key']);
            $field_array['default_value'] = $fields_val['main_taxonomy'];
            build_input_form($field_array);
            unset($field_array);
            ?>
          </div>
      ...
      <div class="row admin_sidebar_box" data-func="hm_content_tags" data-order="<?php echo sidebar_box_order('hm_content_tags'); ?>">
        ...
        <div class="admin_mainbar_boxcontent" data-func="hm_content_tags">
          ...
          <?php
          $field_array = array(
            'nice_name'=>hm_lang('use_available_tag'),
            'name'=>'tag_ids',
            'input_type'=>'taxonomy',
            'data_key'=>array('tag'),
            'required'=>false,
            'default_value'=>$fields_val['tag_ids']
          );
          build_input_form($field_array);
          unset($field_array);
          ?>
