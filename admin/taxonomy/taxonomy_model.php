...
function taxonomy_show_data($key, $status, $perpage, $return_json = TRUE, $parent_tax = 0) {
    ...
    if (!$hmdb->Query("SELECT * FROM " . DB_PREFIX . "taxonomy WHERE `id` IN('$whereIn') $orderBy $limit"))
        $hmdb->Kill();
    if ($hmdb->HasRecords()) {
        /* Trả về các taxonomy */
        while ($row = $hmdb->Row()) {
...
function taxonomy_select_parent($key, $checked_id = 0, $args = array()) {
    ...
    if (isset($args['no_parent'])) {
        $options[] = array(
            'value' => '0',
            'label' => $args['no_parent']
        );
    } else {
        $options[] = array(
            'value' => '0',
            'label' => $this_tax['no_parent']
        );
    }
    while ($row = $hmdb->Row()) {
        $tax_name = '';
