...
function admin_cp_login() {
    ...
    if (is_numeric($logmein)) {
        ...
        $whereArray = array(
            'user_login' => MySQL::SQLValue($user_login)
        );
...
function admin_cp_lostpw() {
    ...
    if (is_numeric($reset_password)) {
        $tableName  = DB_PREFIX . "users";
        $whereArray = array(
            'user_email' => MySQL::SQLValue($user_email)
        );
        $hmdb->SelectRows($tableName, $whereArray);
        if ($hmdb->HasRecords()) {
            $row                   = $hmdb->Row();
            ...
            if ($send_mail) {
                return hm_json_encode(array(
                    'status' => 'success',
                    'mes' => _('Mật khẩu mới đã được gửi về email')
                ));
            } else {
                return hm_json_encode(array(
                    'status' => 'error',
                    'mes' => _('Máy chủ không thể gửi mail xác nhận đổi mật khẩu, vui lòng báo với chủ trang web')
                ));
            }
        } else {
            return hm_json_encode(array(
                'status' => 'error',
                'mes' => _('Email này không khớp với bất kì tài khoản nào')
            ));
        }
    }
}
...
function admin_cp_newpw() {
    ...
    if ($password == $password2) {
        ...
        return hm_json_encode(array(
            'status' => 'success',
            'mes' => _('Đã đổi mật khẩu thành công')
        ));
    } else {
        return hm_json_encode(array(
            'status' => 'error',
            'mes' => _('Hai mật khẩu bạn nhập vào không khớp')
        ));
    }
}
/** Đăng xuất admin cp */
