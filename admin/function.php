...
function admin_menu_con_tax($args = array()) {
    if (current_user_role('content')) {
        ...
        foreach ($all_content as $con) {
            $content_key = $con['content_key'];
            if (!isset($con['content_parent']) OR !$con['content_parent']) {
                ...
                foreach ($all_content as $sub_con) {
                    if (isset($sub_con['content_parent']) AND $sub_con['content_parent'] == $con['content_key']) {
                        echo '<li class="' . $args['sub_li_class'] . '"><a class="material_wave" href="?run=content.php&key=' . $sub_con['content_key'] . '&status=public">' . $sub_con['all_items'] . '</a>' . "\n";
                        echo '<li class="' . $args['sub_li_class'] . '"><a class="material_wave" href="?run=content.php&key=' . $sub_con['content_key'] . '&action=add">' . $sub_con['add_new_item'] . '</a>' . "\n";
                    }
                }
